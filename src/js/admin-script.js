 ...
 *
 * @handle afb-admin
 * @deps jquery,google-chart-api,wp-i18n
 */
		...
		if ( chartArea.length ) {
			...
			$.get( endpoint, function( result ) {
				...
				if ( ! result.ratio.negative ) {
					$( '#afb-pie-chart' ).removeClass( 'loading' ).append( '<p class="no-data">' + __( 'No data', 'anyway-feedback' ) + '</p>' );
				} else {
					const data = new google.visualization.DataTable();
					data.addColumn( 'string', 'Response' );
					data.addColumn( 'number', 'Count' );
					data.addRows( [
						[ __( 'Positive', 'anyway-feedback' ), parseInt( result.ratio.positive ) ],
						[ __( 'Negative', 'anyway-feedback' ), parseInt( result.ratio.negative ) ],
					] );
					// Set chart options
					const options = {
						title: __( 'Feedback Ratio', 'anyway-feedback' ),
						width: '100%',
						height: 300,
				...
				if ( ! result.ranking.length ) {
					$( '#afb-bar-chart' ).removeClass( 'loading' ).append( '<p class="no-data">' + __( 'No data', 'anyway-feedback' ) + '</p>' );
				} else {
					const table = [ [ 'Name', __( 'Positive', 'anyway-feedback' ), __( 'Negative', 'anyway-feedback' ), { role: 'annotation' } ] ];
